...
int ed::menu()
{
 int opcion, posicion;
	...
	PLACE(posicion++,10);
	std::cout << "[3] Representación gráfica de un monomio " << std::endl;
...
void ed::representacionGrafica(){
	...
	std::cout << "Valores mínimo y máximo: ";
	std::cin >> min >> max;

	getValores(m,min,max);

  getGrafica();

  std::cout << BIGREEN;
  std::cout<< "Representación gráfica correcta " << std::endl;
  std::cout << RESET;
